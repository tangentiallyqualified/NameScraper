...
def make_settings(**overrides):
    values = dict(
        ...
        metadata_embed_title=True, mkvmerge_path="",
    )
...
SEASONS = {
    ...
    0: {"titles": {1: "Special"}, "posters": {1: None},
...
def test_tv_plan_slots_and_targets():
    ...
    assert _by_slot(plan, "season_poster:1")["target_relative"] == \
        "Show (2019)/season01-poster.jpg"
    assert _by_slot(plan, "season_poster:0")["target_relative"] == \
        "Show (2019)/season-specials-poster.jpg"

    ep_nfo = _by_slot(plan, "nfo:episode:Show/Show.S01E01.mkv")
    assert ep_nfo["target_relative"] == \
        "Show (2019)/Season 01/Show (2019) - S01E01 - Pilot.nfo"
    thumb = _by_slot(plan, "episode_thumb:Show/Show.S01E01.mkv")
    assert thumb["target_relative"] == \
        "Show (2019)/Season 01/Show (2019) - S01E01 - Pilot-thumb.jpg"
...
def test_plex_naming_adds_duplicate_targets():
    ...
    assert all(a["slot"] in ("season_poster:1", "season_poster:0",
...
def test_movie_plan():
    op = RenameOp(
        ...
        status="OK")
    job = tv_job(ops=[op], media_type=MediaType.MOVIE, tmdb_id=7,
                 show_folder_rename="Dune (2021)")
    movie = {"id": 7, "title": "Dune", "overview": "Sand.",
             "release_date": "2021-09-15", "poster_path": "/mp.jpg",
             "backdrop_path": "/mb.jpg", "images": {"logos": []}}
...
def test_finalize_drops_placeholders_and_empty_plans():
    plan = {"nfo_files": [], "artwork": [
        {"tmdb_path": None, "target_relative": "x/a.jpg",
         "kind": "episode_thumb", "slot": "episode_thumb:x", "plex_extra": False},
...
def test_no_plan_without_master_or_client_or_id():
    ...
    assert build_metadata_plan(
        tv_job(), FakeTMDB(), make_settings(metadata_enabled=False)) is None
    assert build_metadata_plan(
        tv_job(tmdb_id=0), FakeTMDB(), make_settings()) is None
