...
class TMDBClientTests(unittest.TestCase):
    ...
    def test_cache_snapshot_roundtrip_restores_runtime_metadata(self):
        ...
        self.assertEqual(exported["season_map_contract_version"], 1)
        client._session.close()

    def test_legacy_snapshot_discards_only_poisoned_season_maps_and_refetches(self):
        client = TMDBClient("dummy-api-key")
        client._transport = MagicMock()
        client._transport.get_json.side_effect = [
            {"seasons": [{"season_number": 1, "episode_count": 1, "name": "Season 1"}]},
            {"episodes": [{"episode_number": 1, "name": "Fresh Pilot"}]},
        ]
        legacy_snapshot = {
            "show_cache": {"321": {"poster_path": "/andor.jpg", "name": "Andor"}},
            "season_cache": {
        ...
        self.assertEqual(client.get_cached_poster_path(321), "/andor.jpg")
        self.assertEqual(client.get_cached_poster_path(123, media_type="movie"), "/movie.jpg")
        self.assertEqual(client.get_season(321, 1)["titles"][1], "Cached single season")
        season_map, total_episodes = client.get_season_map(321)
        self.assertEqual(total_episodes, 1)
        self.assertEqual(season_map[1]["titles"], {1: "Fresh Pilot"})
        self.assertEqual(client._transport.get_json.call_count, 2)
        client._session.close()
