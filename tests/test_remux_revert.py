...
def _job(tmp_path, undo):
    return RenameJob(
        ...
        source_folder="Show", job_kind=JobKind.REMUX, undo_data=undo)
...
def test_revert_deletes_remux_outputs(tmp_path):
    ...
    job = _job(tmp_path, {
        "renames": [], "created_dirs": [], "removed_dirs": [],
        "renamed_dirs": [], "remux_outputs": [str(final)]})
...
def test_no_fear_job_is_irreversible(tmp_path):
    ...
    final.write_bytes(b"muxed")
    job = _job(tmp_path, {
        "renames": [], "remux_outputs": [str(final)], "irreversible": True})
    ok, errors = revert_job(job)
    assert not ok
