...
SEASONS = {
    ...
    2: {"titles": {}, "posters": {}, "episodes": {}, "season_poster_path": None},
}
...
def test_tv_cover_falls_back_to_show_poster():
    ...
    plan = build_metadata_plan(
        tv_job(ops=ops), FakeTMDB(details=TV_DETAILS, seasons=SEASONS), make_settings()
    )
...
def test_movie_entry_uses_movie_poster_and_tags():
    plan = build_metadata_plan(movie_job(), FakeTMDB(movie=MOVIE_DETAILS), make_settings())
...
def test_toggles_gate_fields_independently():
    ...
    cover_only = build_metadata_plan(tv_job(), tmdb, make_settings(metadata_embed_tags=False))
    entry = _extras(cover_only)["Show/Show.S01E01.mkv"]
    assert entry["tags_xml"] is None
    assert entry["cover_tmdb_path"] == "/s1.jpg"

    tags_only = build_metadata_plan(tv_job(), tmdb, make_settings(metadata_embed_cover=False))
    entry = _extras(tags_only)["Show/Show.S01E01.mkv"]
    assert entry["cover_tmdb_path"] is None
    assert "Pilot" in entry["tags_xml"]

    neither = build_metadata_plan(
        tv_job(), tmdb, make_settings(metadata_embed_cover=False, metadata_embed_tags=False)
    )
...
def test_finalize_plan_survives_on_extras_alone():
    plan = build_metadata_plan(
        movie_job(),
        FakeTMDB(movie=MOVIE_DETAILS),
        make_settings(
            metadata_write_nfo=False,
            metadata_write_poster=False,
