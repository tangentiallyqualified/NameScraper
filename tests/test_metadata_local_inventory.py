...
def test_inventory_maps_slots(tmp_path):
    ...
    found = inventory_local_metadata(
        src, ops, MediaType.TV, tmp_path / "src")
    ...
    assert found["episode_thumb:Show/Show.S01E01.mkv"].name == \
...
def test_prefer_local_carries_and_removes_plan_entries(tmp_path):
    ...
    plan = build_metadata_plan(
        job, FakeTMDB(details=TV_DETAILS, seasons=SEASONS), settings)
    ...
    for fulfilled in ("poster", "fanart", "season_poster:1", "nfo:show",
    ...
    assert thumb_op.original_relative.replace("\\", "/") == \
        "Show/Show.S01E01-thumb.jpg"
...
def test_always_tmdb_leaves_plan_untouched(tmp_path):
    ...
    job = tv_job(library_root=str(tmp_path / "src"))
    plan = build_metadata_plan(
        job, FakeTMDB(details=TV_DETAILS, seasons=SEASONS), make_settings())
...
def test_local_fulfills_slot_tmdb_lacks(tmp_path):
    ...
    plan = build_metadata_plan(
        job, FakeTMDB(details=TV_DETAILS, seasons=SEASONS), settings)
    apply_prefer_local(job, plan, tmp_path / "src")
    carried = [op for op in job.rename_ops
               if op.new_name == "Show (2019) - S00E01 - Special-thumb.jpg"]
