...
from plex_renamer._job_revert import RevertContext, destination_path_errors
...
def test_output_and_directory_handlers_match_public_revert_order_and_tree(
    ...
    assert hasattr(job_revert, "remove_generated_outputs"), (
        "remove_generated_outputs handler is not implemented"
    )
    assert hasattr(job_revert, "restore_directories"), (
        "restore_directories handler is not implemented"
    )
    ...
    def setup_tree(root: Path) -> RenameJob:
        ...
        undo: dict[str, object] = {
            ...
            "renamed_dirs": [
                ...
                {
                    "new": str(renamed_season),
                    "old": str(renamed_root / "S1"),
                },
            ],
        }
        return _job(root, undo=undo, output=False)
    ...
    assert direct_job.undo_data is not None
    context = RevertContext(
        job=direct_job,
        undo=direct_job.undo_data,
