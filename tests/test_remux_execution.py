...
def test_success_writes_final_and_keeps_sources(tmp_path):
    ...
    ok = rex.execute_remux_op(
        op, source_root=lib, output_root=out, result=result,
        on_percent=percents.append, runner=_ok_runner)
...
def test_run_mkvmerge_parses_progress():
    script = (
        "import sys;"
        "print('Progress: 25%');print('Progress: 100%');print('done')")
    percents = []
    code, tail = rex.run_mkvmerge(
        [sys.executable, "-c", script], on_percent=percents.append)
    assert code == 0
