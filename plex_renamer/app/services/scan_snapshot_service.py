...
class ScanSnapshotService:
    ...
    def _serialize_state(self, state: ScanState) -> dict[str, Any]:
        return {
            ...
            "relative_folder": state.relative_folder,
            "parent_relative_folder": state.parent_relative_folder,
            "duplicate_of_relative_folder": state.duplicate_of_relative_folder,
            "discovery_reason": state.discovery_reason,
            "has_direct_season_subdirs": state.has_direct_season_subdirs,
            "direct_episode_file_count": state.direct_episode_file_count,
            "direct_video_file_count": state.direct_video_file_count,
            "discovered_via_symlink": state.discovered_via_symlink,
    ...
    def _deserialize_state(self, payload: dict[str, Any]) -> ScanState:
        ...
        state = ScanState(
            ...
            parent_relative_folder=payload.get("parent_relative_folder"),
            duplicate_of_relative_folder=payload.get("duplicate_of_relative_folder"),
            discovery_reason=payload.get("discovery_reason", ""),
            has_direct_season_subdirs=bool(payload.get("has_direct_season_subdirs", False)),
            direct_episode_file_count=int(payload.get("direct_episode_file_count", 0)),
            direct_video_file_count=int(payload.get("direct_video_file_count", 0)),
            discovered_via_symlink=bool(payload.get("discovered_via_symlink", False)),
            scanned=scanned,
            scanning=scanning,
