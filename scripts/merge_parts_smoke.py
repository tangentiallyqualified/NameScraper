...
mkvmerge append pipeline, and verifies the merged output's duration,
track layout, and subtitle offset. Requires ffmpeg, ffprobe, and mkvmerge
on this machine.
...
def _need(binary: str) -> str | None:
    ...
    if found is not None:
        return found
    # Fallback to mkvmerge installation path
    if binary == "mkvmerge":
        default = Path(r"C:\Program Files\MKVToolNix\mkvmerge.exe")
        return str(default) if default.is_file() else None
    # Fallback to ffmpeg/ffprobe winget installation path
...
def main() -> int:
    ...
    if not ffmpeg or not ffprobe or not mkvmerge:
        print(f"missing binaries: ffmpeg={ffmpeg} ffprobe={ffprobe} mkvmerge={mkvmerge}")
        return 2
    ...
    try:
        ...
        # Verify subtitle offset: part 2 cue starts at 0.5s, part 1 is 4s, so
        # first subtitle packet in merged file must sit at ~4.5s (±0.3s tolerance).
        ...
        first_line = output.stdout.strip().split("\n")[0]
        first_pts = float(first_line)
        assert 4.2 <= first_pts <= 4.8, (
            f"subtitle offset broken: first packet at {first_pts}s, expected ~4.5s"
        )

        # Success: clean up tempdir.
        shutil.rmtree(root)
        print(
            f"OK: merged {len(parts)} parts -> {final} ({merged.duration_ms} ms, "
            f"first_subtitle_pts={first_pts:.2f}s)"
        )
        return 0
    except Exception:
        print(f"ERROR: failed, leaving debug tempdir at: {root}")
        raise
