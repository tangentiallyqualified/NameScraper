...
def _engine_cycle_edges(graph: Mapping[str, object]) -> list[tuple[str, str]]:
    prefix = "plex_renamer.engine."
    cycles = cast(list[dict[str, object]], graph.get("cycles", []))
    return sorted(
        (source, target)
        for cycle in cycles
        for source, target in cast(list[tuple[str, str]], cycle.get("edges", []))
        if source.startswith(prefix) and target.startswith(prefix)
    )


def _validated_record(record: object, index: int) -> CycleEdgeClassification:
    if not isinstance(record, dict):
        raise ValueError(f"cycle edge classification {index} must be a table")
    raw_record = cast(dict[str, object], record)
    ...
    if raw_record["disposition"] not in CYCLE_EDGE_DISPOSITIONS:
        allowed = ", ".join(sorted(CYCLE_EDGE_DISPOSITIONS))
        raise ValueError(
            f"cycle edge classification {index} has unsupported disposition "
            f"{raw_record['disposition']!r}; expected one of: {allowed}"
        )
    return CycleEdgeClassification(
        source=cast(str, raw_record["source"]),
        target=cast(str, raw_record["target"]),
        owner=cast(str, raw_record["owner"]),
        purpose=cast(str, raw_record["purpose"]),
        disposition=cast(str, raw_record["disposition"]),
    )


def _validate_edge_coverage(
    records: list[CycleEdgeClassification], graph: Mapping[str, object]
) -> None:
