...
- [ ] Plans 4–9: NOT written yet — write each with `superpowers:writing-plans` when its predecessor lands (see roadmap standing rules)

## How to resume

1. **Next step: execute Plan 3** with `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans`. Task 5 (the cutover) is the largest single task of the roadmap — give it a dedicated capable agent.
