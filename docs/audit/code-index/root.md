...
- Tests: tests/conftest_qt.py, tests/test_alt_title_matching_orchestrator.py, tests/test_ffprobe_fallback.py, tests/test_keys.py, tests/test_mkv_locate.py, tests/test_mkv_locate_fallbacks.py, tests/test_mkv_probe.py, tests/test_mkvmerge_integration.py, tests/test_queue_executor_progress.py, tests/test_remux_append_execution.py, tests/test_remux_execution.py, tests/test_thread_pool.py

### `plex_renamer/__main__.py` — Entry point for NameScraper.
- `main()` — (no docstring)

### `plex_renamer/_ffprobe.py` — Optional ffprobe helper: per-stream audio bitrates for containers
- `find_ffprobe(explicit) -> Path | None` — Resolve the ffprobe binary: an explicit path if it is a real file, (used by: plex_renamer.app.services.automux_service)
- `probe_audio_bitrates(ffprobe, video_path, runner) -> list[int]` — Bitrate per audio stream in stream order; 0 unknown; [] on failure. (used by: plex_renamer._mkv_probe)
- Tests: tests/test_ffprobe_fallback.py
...
### `plex_renamer/_job_revert.py` — Per-job rollback orchestration and filesystem helpers.
- `RevertContext` — (no docstring)
- `destination_path_errors(*, new_path, old_path, output_boundary, source_boundary) -> list[str]` — (no docstring)
- `remap_after_directory_revert(path, mapping) -> Path` — (no docstring)
- `remove_generated_outputs(context) -> None` — (no docstring)
- `restore_directories(context) -> None` — (no docstring)
- `restore_files(context) -> None` — (no docstring)
- `cleanup_reverted_tree(context) -> None` — (no docstring)
- `revert_job(job) -> tuple[bool, list[str]]` — Revert a single completed job using its stored undo data. (used by: plex_renamer.job_executor)
- Tests: tests/test_job_revert_characterization.py, tests/test_job_revert_handlers.py
...
- `strip_provider_id_tags(name) -> str` — *name* with every provider-ID tag removed and whitespace collapsed. (used by: plex_renamer._parsing_titles, plex_renamer.parsing)
- Tests: tests/test_id_tag_parsing.py
...
### `plex_renamer/_provider_errors.py` — Provider-neutral exceptions shared without importing the provider registry.
...
### `plex_renamer/_tmdb_snapshot_validation.py` — Strict normalization helpers for persisted TMDB season-map snapshots.
- `normalize_season_map_snapshot(season_map) -> SeasonMap` — Validate and normalize one persisted season map.
- `iter_valid_season_map_snapshots(value) -> Iterator[SeasonMapCacheEntry]` — Yield valid cache entries while skipping malformed entries independently. (used by: plex_renamer._tmdb_metadata_cache)

### `plex_renamer/_tmdb_transport.py` — Transport helpers for TMDB client networking and retry behavior.
- `TMDBError` — Base class for TMDB client errors. (used by: plex_renamer._tmdb_season_map, plex_renamer._tvdb_payloads, plex_renamer._tvdb_transport, plex_renamer.tmdb, plex_renamer.tvdb)
...
- Tests: tests/season_map_test_support.py, tests/test_tmdb_transport.py, tests/test_tvdb.py

### `plex_renamer/_tvdb_payloads.py` — Typed validation and strict fetching for untrusted TVDB payloads.
- `validated_record_list(value) -> list[dict[str, Any]] | None` — Return a typed view of a list containing only mapping records. (used by: plex_renamer.tvdb)
- `validated_series_details(value) -> dict[str, Any] | None` — Return details only when every nested record used by normalization is safe.
- `optional_series_details(response) -> dict[str, Any] | None` — Extract a valid details payload from a best-effort TVDB response. (used by: plex_renamer.tvdb)
- `fetch_series_details_strict(transport, show_id) -> dict[str, Any]` — Fetch a trustworthy extended-details payload or raise the provider error. (used by: plex_renamer.tvdb)
- `fetch_all_episodes_strict(transport, show_id) -> list[dict[str, Any]]` — Fetch and validate every TVDB episode page without caching partial data. (used by: plex_renamer.tvdb)

### `plex_renamer/_tvdb_transport.py` — HTTP transport for TheTVDB v4 API: bearer auth, JSON GETs, image bytes.
- `TVDBTransport` — Logs in lazily on first request; re-logs-in once on 401 (token expiry). (used by: plex_renamer._tvdb_payloads, plex_renamer.tvdb)
...
- Tests: tests/test_executor_metadata_integration.py, tests/test_job_revert_characterization.py, tests/test_job_revert_handlers.py, tests/test_queue_executor_progress.py, tests/test_remux_embed_extras.py, tests/test_remux_revert.py, tests/test_scan_improvements.py
...
- `RenameJob` — A single unit of work in the job queue. (used by: plex_renamer._job_revert, plex_renamer.app.controllers, plex_renamer.app.controllers._queue_history_helpers, plex_renamer.app.controllers._queue_submission_helpers, plex_renamer.app.controllers.queue_controller, plex_renamer.engine._queue_bridge, plex_renamer.gui_qt._main_window_feedback, plex_renamer.gui_qt.main_window, plex_renamer.gui_qt.models.job_table_model, plex_renamer.gui_qt.widgets._job_detail_data, plex_renamer.gui_qt.widgets._job_detail_poster, plex_renamer.gui_qt.widgets._job_detail_preview, plex_renamer.gui_qt.widgets._job_list_tab, plex_renamer.gui_qt.widgets.job_detail_panel, plex_renamer.job_executor)
...
- Tests: tests/conftest_qt.py, tests/test_executor_metadata_integration.py, tests/test_job_execution_metadata.py, tests/test_job_preview_grouping.py, tests/test_job_revert_characterization.py, tests/test_job_revert_handlers.py, tests/test_job_store_metadata_plan.py, tests/test_job_store_provider_provenance.py, tests/test_media_controller.py, tests/test_metadata_embed_extras.py, tests/test_metadata_service.py, tests/test_mkvmerge_integration.py, tests/test_qt_job_detail_panel.py, tests/test_qt_main_window.py, tests/test_qt_queue_history.py, tests/test_queue_controller.py, tests/test_queue_executor_progress.py, tests/test_queue_tab_remux.py, tests/test_remux_append_execution.py, tests/test_remux_confirmation.py, tests/test_remux_embed_extras.py, tests/test_remux_execution.py, tests/test_remux_job_model.py, tests/test_remux_revert.py, tests/test_scan_improvements.py, tests/test_tv_provider_selection.py
...
- `MetadataProvider` — Structural port for TV metadata clients (TMDB today, TVDB, ...). (used by: plex_renamer.engine._batch_orchestrators, plex_renamer.engine._batch_tv_match_policy, plex_renamer.engine._tv_scanner, plex_renamer.engine._tv_score_fallback, plex_renamer.engine.matching)
